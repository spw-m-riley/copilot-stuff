...
122. [COMMUNICATION] Never claim Matt manually pasted skill bodies when prompts contain `<skill-context ...>` blocks; treat those as runtime/plugin-injected context unless he explicitly says he pasted content himself - this correction flagged a wrong attribution in cost-analysis guidance
123. [WORKFLOW] When a background process (e.g. a session-store reindex) surfaces an explicit error/failure count alongside a vague prompt like "What happened?", root-cause that concrete failure first via logs or diagnostic tools instead of continuing unrelated exploratory queries that keep returning empty results - Matt had to stop an investigation that chased empty session/todo queries instead of the actual reindex error counts
124. [COMMUNICATION] Once grep/strings/log searches surface enough raw evidence to answer a direct factual question (e.g. "what is X"), stop digging and synthesize a plain-English answer in the same turn instead of ending on tool output or drifting to a related but different topic - Matt had to ask "What are those 'Mission Control' things?" twice because I gathered relevant hits (changelog strings, RPC doc comments) but never converted them into a direct answer
125. [EXTENSIONS] `/agent <name>` joins every token after the command into one string used as the exact agent lookup id (`e.join(" ").trim()` in the CLI source) — it does NOT parse "agent name" separately from "prompt to send it." Appending a prompt on the same line (e.g. `/agent research-fleet do the research task...`) makes the whole trailing text the lookup string, which matches no agent and reports "Custom agent not found: <name + prompt>". Correct usage is always two steps: send `/agent <name>` alone first, confirm the switch (status bar shows the agent name), then send the task as a separate follow-up message
126. [SHELL] `herdr pane run <pane> "<text>"` can leave text sitting unsubmitted in a pane's input box instead of actually submitting it; always re-read the pane immediately after sending to confirm the text left the input box and the process started working, and use `herdr pane send-keys <pane> Enter` to submit if it didn't - a batch of sends all returning success does not mean they all landed
127. [SHELL] When using `herdr wait output <pane> --match <marker>`, do not trust a match if the marker text is also present verbatim in the instruction you just sent to that pane - the wait can match the echoed prompt itself the instant it's submitted, long before the sub-agent actually finishes; cross-check pane status or the marker's position in the transcript before treating the task as complete
128. [GIT] After rewriting history with `git filter-repo` in a scratch clone, always re-add the remote (for example `origin`) before fetching or force-pushing - `git filter-repo` strips remotes as a safety measure, and forgetting that makes the final push fail even when the rewrite itself succeeded
128. [GIT] Stashing an unrelated uncommitted change with `git stash push -- <file>` does not protect it from a later `git checkout <other-branch>`; uncommitted working-tree state is tied to the working directory, not a branch, so popping the stash back onto its home branch and then checking out a different branch again will carry the same diff along if it's re-stashed and popped without re-stashing first. Always re-stash any unrelated file immediately before doing further work on another branch, and only pop it back once fully done switching branches - this session had to re-discover and re-stash the same unrelated `lib/capability-utils.mjs` diff twice while alternating between a feature branch and the branch that actually owned the change
129. [WORKFLOW] When imported `golang-*` skills regress to upstream frontmatter with nested arrays (`metadata.openclaw.requires.bins: - go`), treat it as a two-part repair: normalize frontmatter to the local contract and re-add the required reference-skill headings (`## Use this skill when`, `## Do not use this skill when`, `## Validation`, `## Examples`, `## Reference files`) with explicit support-file links before rerunning the full validator - this session showed that fixing only frontmatter still leaves those skills failing heading/reference checks
130. [GIT] The active `gh` account (`gh auth switch`) can silently revert to a different logged-in account between tool calls in this shared environment, even though the switch persists to a config file; never assume a prior `gh auth switch` is still in effect for a `gh api`/`gh pr` call against a repo that requires a specific account (e.g. `matt-riley/lore` needing `spw-m-riley`) - always re-check `gh auth status` (or just re-run the switch) immediately before the operation if the first attempt fails with an Enterprise Managed User/access error
135. [MEMORY] When adding the next Lore local-inference configuration change, provide a persistent default opt-in for model-backed `lore_reflect` calls while keeping that setting false by default - Matt wants to enable Gemma once in config instead of explicitly requesting it on every reflection
136. [MEMORY] Never treat `localInference: used (embeddings: used)` as proof that a Lore reflection is high quality; compare Gemma's conclusions with the rendered supporting evidence and report synthesis drift plainly - the first live cross-repo Git/CI reflection retrieved relevant workflow evidence but Gemma ignored it and substituted unrelated activity
138. [GIT] When verifying SSH-signed Git commits without `gpg.ssh.allowedSignersFile`, do not trust `git log --show-signature` reporting `No signature`; inspect the commit object for a `gpgsig` SSH signature block, and treat cryptographic identity verification as unavailable until the trusted allowed-signers configuration exists - this session nearly rebuilt a correctly signed commit because the verifier configuration was missing
139. [WORKFLOW] When live-testing a tool handler directly, pass the structured arguments that the natural-language request implies before diagnosing routing from rendered output - this session initially misread `cross_repo_lookup_disabled` as a routing bug because the direct `lore_reflect` probe omitted `includeOtherRepositories: true`
