...
description: "Use when review go database scanning or transaction code; not when another Go skill is a better fit."
metadata:
  category: go
...
# Imported Go Database Guide

Use this skill when you are writing or reviewing Go database access, scanning, transactions, or test fixtures.
...
| --- | --- | --- |
| The request is specifically about go database access. | Yes | - |
| The request is better served by an adjacent Go skill. | No | Route optimization questions to [`golang-performance`](../../golang-performance/SKILL.md) and debugging questions to [`golang-troubleshooting`](../../golang-troubleshooting/SKILL.md). |

## Guardrails

- Keep the guidance focused on go database access work.
...
- Smoke test:
  - should trigger: "Review Go database scanning or transaction code."
  - should not trigger: "Optimize a Go benchmark result."
...
- [`references/performance.md`](./performance.md)
- [`references/scanning.md`](./scanning.md)
...
Use `db:"column_name"` tags for sqlx, `pgx.CollectRows` with `pgx.RowToStructByName` for pgx. Handle NULLable columns with pointer fields (`*string`, `*time.Time`) — they work cleanly with both scanning and JSON marshaling. See [Scanning Reference](./scanning.md) for examples of all approaches.
...
For transaction patterns, isolation levels, `SELECT FOR UPDATE`, and locking variants, see [Transactions](./transactions.md).
...
For sizing guidance and formulas, see [Database Performance](./performance.md).
...
- **[Transactions](./transactions.md)** — Transaction boundaries, isolation levels, deadlock prevention, `SELECT FOR UPDATE`
- **[Testing Database Code](./testing.md)** — Mock connections, integration tests with containers, fixtures, schema setup/teardown
- **[Database Performance](./performance.md)** — Connection pool sizing, batch processing, indexing strategy, query optimization
- **[Struct Scanning](./scanning.md)** — Struct tags, NULLable column handling, JSON marshaling patterns
