...
| Run logs show `##[warning] Node.js 20 is deprecated` from setup-node or configure-nodejs | Action version or runtime deprecation warnings | Read the exact warning; identify which workflow file and step pins the deprecated version | Update the action version or `node-version` input; rerun to confirm warnings are gone |
