...
| Action version or runtime deprecation warnings | `##[warning]` annotations in run logs about deprecated Node.js versions (e.g. Node 20), deprecated action major versions, or end-of-life runtimes; warnings from `setup-node`, `configure-nodejs`, or similar setup actions | Read the exact warning annotation; identify the pinned action version or `node-version` input that needs updating; treat as high priority when a removal date is announced or the warning blocks a required check |
