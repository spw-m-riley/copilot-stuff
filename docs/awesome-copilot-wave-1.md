...
| Skill | `agent-governance` | Adds reusable governance patterns for tool access, approval gates, audit trails, and fail-closed behavior. | Initially installed as a user-scope skill in the live main checkout; later tracked in this repository, then merged with `agent-supply-chain` into the consolidated [`./skills/agent-safety/`](../skills/agent-safety/) package (governance section). |
| Skill | `agent-supply-chain` | Adds integrity-manifest and verification patterns for reviewing third-party agent/plugin content. | Initially installed as a user-scope skill in the live main checkout; later tracked in this repository, then merged with `agent-governance` into the consolidated [`./skills/agent-safety/`](../skills/agent-safety/) package (supply-chain section). |
