...
4. [ACTIONS] Never set a fake `CIRCLECI` environment variable in GitHub Actions workflows to drive script behavior; prefer the standard `CI` signal or an explicit repo-owned variable with a truthful name - the user explicitly said `CIRCLECI` should not be mentioned for this compatibility case
