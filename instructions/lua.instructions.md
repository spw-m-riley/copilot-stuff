...
31. [NEOVIM] When the user scopes a Lua/Neovim change to the smallest truthful implementation lane, stop extra doc or runtime probing once the required wiring is clear and ship the scoped change with the agreed validation commands - this correction explicitly prioritized implementation over more Progress-payload research
32. [NEOVIM] Never use regex alternation syntax like `(get|post)` in Lua patterns; use an explicit lookup table or separate matches instead - Lua patterns do not support `|`, and this session's APIKit method parser silently rejected every valid generated flow name
33. [NEOVIM] When validating this Neovim config from a nested worktree whose directory is not literally named `nvim`, set `XDG_CONFIG_HOME` to the worktree parent, `NVIM_APPNAME` to the worktree directory name, and isolate `XDG_DATA_HOME`/`XDG_STATE_HOME`/`XDG_CACHE_HOME` to temp paths - this session's first command-smoke review loaded the live config and missed the worktree-only `commands` stage
34. [NEOVIM] When running many headless Neovim smoke stages in one invocation, use a single `+lua` loop instead of one `+lua` argument per stage - Neovim rejects too many `+command`/`-c` arguments before the validation reaches the actual code
